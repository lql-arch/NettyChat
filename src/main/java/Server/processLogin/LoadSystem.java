...
public class LoadSystem{
    ...
    public static LoadMessage loadMessage(String uid) throws SQLException {
        ...
        //得到好友的uid
        ps2 = conn.prepareStatement("select first_uid from friends where second_uid = ?");
        ps2.setObject(1,uid);
        rs = ps2.executeQuery();
        while(rs.next()){
            String friend = rs.getString("first_uid");
            friends.add(friend);
        }
        ps2 = conn.prepareStatement("select second_uid from friends where first_uid = ?");
        ps2.setObject(1,uid);
        rs = ps2.executeQuery();
        while(rs.next()){
        ...
        //得到自己的资料
        ps2 = conn.prepareStatement("select name,age,gander,build_time from user where uid = ?");
        ps2.setObject(1,uid);
        rs = ps2.executeQuery();
        while(rs.next()){
        ...
        ps2 = conn.prepareStatement("select text,send_uid,time,status from user_text where recipient_uid = ? ");
        ps2.setObject(1, uid);
        rs = ps2.executeQuery();
        while (rs.next()) {
    ...
    public static UserMessage friendMaterial(String uid) throws SQLException {
        ...
        ps = conn.prepareStatement("select name,age,build_time,gander from user where uid = ?");
        ps.setObject(1,uid);
        rs = ps.executeQuery();
        while(rs.next()){
    ...
    public static LoadMessage SingleChat(String myUid,String friendUid) throws SQLException {
        ...
        ps = conn.prepareStatement("select text,recipient_uid,send_uid,time,status from user_text where recipient_uid = ? or send_uid = ?");
        ps.setObject(1, myUid);
        ps.setObject(2,friendUid);
        unread_message = getUnread_message(myUid, ps, unread_message, message);

        ps = conn.prepareStatement("select text,recipient_uid,send_uid,time,status from user_text where recipient_uid = ? or send_uid = ?");
        ps.setObject(2, myUid);
        ps.setObject(1,friendUid);
        unread_message = getUnread_message(myUid, ps, unread_message, message);
